 ...
 * Description: 投资详情()
 */
public class RepaymentDetailVM extends BaseRecyclerViewVM<BaseListItemMo> {
    ...
    public RepaymentDetailVM(CommonRecyclerViewBinding binding, String investmentId) {
    ...
    private List<BaseListItemMo> init(final InvestRecordDetailMo mo) {
        ...
        if (mo.getClassify() != 4) {
            list.add(new BaseListItemMo(Constant.NUMBER_1, "查看协议", "", new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Log.e("show data", mo.getBorrowId() + " + " + investmentId);
                    getBorrowProtocol(mo.getBorrowId(), investmentId);
                }
            }));
        }
        return list;
    ...
    private void getBorrowProtocol(String borrowId, String investmentId) {
        Call<ProtocolMo> call = RDClient.getService(ProductService.class).getBorrowProtocolTwo(borrowId, investmentId);
        call.enqueue(new RequestCallBack<ProtocolMo>() {
            ...
            public void onSuccess(Call<ProtocolMo> call, Response<ProtocolMo> response) {
                ...
                intent.putExtra(BundleKeys.LOADPROTOCOL,1);
